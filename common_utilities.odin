...
split_params :: proc(from_route: string) -> (path: string, params: string) {
  pos := strings.index_byte(from_route, '?')
  if pos == -1 do pos = len(from_route)
  return from_route[1:pos], from_route[pos:]
}
